...
export function renderChatRoom(roomId, initialData) {
  ...
  var css = [
    ...
    '.we-onboard-player{display:flex;align-items:center;justify-content:center;gap:10px;font-size:13px;font-weight:700;color:#888}',
    '.we-onboard-dot{width:10px;height:10px;border-radius:50%}',
    // Player bar (shown after onboarding)
  ...
  var js = [
    ...
    '  var overlay=document.getElementById("we-onboard");',
    ...
    '    var d=await r.json();',
    '    if(d.ok){',
    '      myName=d.display_name;myColor=d.color;myPlayerNum=d.player_number;',
    '      hasJoined=true;saveIdentity();',
    '      colorMap[myParticipantId]=myColor;',
    '      hideOnboarding();',
    '      updatePlayerBar(d.participants||[]);',
    '      poll();',
    '    } else {',
    ...
    '    chip.className="we-player-chip"+(p.participant_id===myParticipantId?" me":"");',
    '    var dot=document.createElement("div");',
    '    dot.className="we-player-chip-dot";',
    '    dot.style.background=p.color||(COLORS[(p.player_number-1)%COLORS.length]);',
    '    colorMap[p.participant_id]=p.color;',
    '    var label=document.createElement("span");',
    '    label.textContent="P"+p.player_number+" "+p.display_name+(p.participant_id===myParticipantId?" (you)":"");',
    ...
    '    ctaBtn.addEventListener("click",function(){if(cards.cta_action==="reserve"){var p=document.getElementById("lead-panel");if(p){p.classList.add("visible");p.scrollIntoView({behavior:"smooth",block:"nearest"}); }}});',
    ...
    '  var bubbleClass=isSystem?"system":isAssistant?"assistant":isSelf?"user-self":"user-other";',
    '  var wrap=document.createElement("div");wrap.className="we-msg-wrap "+wrapClass;',
    '  var bubble=document.createElement("div");bubble.className="we-bubble "+bubbleClass;',
    ...
    '    // Also refresh player bar periodically
',
    '    fetch("/api/chat/room/participants?id="+encodeURIComponent(ROOM_ID))',
    '    .then(function(r2){return r2.json();})',
    '    .then(function(d2){if(d2.ok&&d2.participants)updatePlayerBar(d2.participants);})',
    '    .catch(function(){});',
    '  })',
    '  .catch(function(){});',
    '}',
    ...
    '        renderCardBubble(ad.reply_cards,new Date().toISOString(),ad.message_id);',
    '      } else if(ad.reply){',
    ...
    '  fetch("/api/chat/room/participants?id="+encodeURIComponent(ROOM_ID)).then(function(r){return r.json();}).then(function(d){if(d.ok)updatePlayerBar(d.participants);}).catch(function(){});',
    '} else {',
    '  showOnboarding();',
    '  renderMessages(INITIAL_MSGS);',
    '}',
